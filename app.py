...
# NEW: Prompts Circle Routes
@app.route('/prompts_circle')
@login_required
def prompts_circle():
    # Fetch messenger settings
    settings = MessengerSetting.query.first()
    if not settings:
        # Create default settings if they don't exist
        settings = MessengerSetting(allow_posting=True, moderation_enabled=False, disallowed_keywords="")
        db.session.add(settings)
        db.session.commit()

    # Fetch messages based on moderation settings
    if settings.moderation_enabled:
        messages = Message.query.filter_by(is_approved=True).order_by(Message.timestamp.desc()).limit(50).all()
    else:
        messages = Message.query.order_by(Message.timestamp.desc()).limit(50).all()

    return render_template('prompts_circle.html', messages=messages, current_user=current_user, settings=settings)

@app.route('/prompts_circle/send_message', methods=['POST'])
@login_required
def prompts_circle_send_message():
    settings = MessengerSetting.query.first()
    if not settings or not settings.allow_posting:
        return jsonify({"success": False, "message": "Posting is currently disabled by the administrator."}), 403
    ...
    is_approved = True
    if settings.moderation_enabled and settings.disallowed_keywords:
        disallowed_list = [k.strip().lower() for k in settings.disallowed_keywords.split(',') if k.strip()]
        for keyword in disallowed_list:
    ...
    try:
        ...
        if settings.moderation_enabled and not is_approved:
            return jsonify({"success": True, "message": "Message sent for moderation. It will appear after approval."})
        else:
            return jsonify({"success": True, "message": "Message sent!"})
    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Error sending message to prompts circle: {e}")
        return jsonify({"success": False, "message": "An error occurred while sending your message."}), 500

@app.route('/prompts_circle/get_messages', methods=['GET'])
@login_required
def prompts_circle_get_messages():
    settings = MessengerSetting.query.first()
    if not settings: # Fallback if settings not initialized
        settings = MessengerSetting(allow_posting=True, moderation_enabled=False, disallowed_keywords="")

    if settings.moderation_enabled:
        messages = Message.query.filter_by(is_approved=True).order_by(Message.timestamp.asc()).all() # Fetch approved only
    else:
        messages = Message.query.order_by(Message.timestamp.asc()).all() # Fetch all if no moderation
    ...
    return jsonify({"messages": messages_data, "settings": {"allow_posting": settings.allow_posting, "moderation_enabled": settings.moderation_enabled}})
...
# NEW: Admin Messenger Settings Routes
@app.route('/admin/messenger_settings', methods=['GET'])
@admin_required
def admin_messenger_settings():
    settings = MessengerSetting.query.first()
    if not settings:
        settings = MessengerSetting(allow_posting=True, moderation_enabled=False, disallowed_keywords="")
        db.session.add(settings)
    ...
    # Fetch all messages for moderation, ordered by timestamp (newest first)
    messages = Message.query.order_by(Message.timestamp.desc()).all()
    
    return render_template('admin_messenger_settings.html', settings=settings, messages=messages, current_user=current_user)

@app.route('/admin/messenger_settings/update', methods=['POST'])
@admin_required
def admin_messenger_settings_update():
    settings = MessengerSetting.query.first()
    if not settings:
        settings = MessengerSetting() # Create if not exists
        db.session.add(settings)
    
    settings.allow_posting = 'allow_posting' in request.form
    settings.moderation_enabled = 'moderation_enabled' in request.form
    settings.disallowed_keywords = request.form.get('disallowed_keywords', '').strip()

    try:
        db.session.commit()
        flash('Messenger settings updated successfully!', 'success')
    except Exception as e:
        db.session.rollback()
        flash(f'Error updating messenger settings: {e}', 'danger')
        app.logger.error(f"Error updating messenger settings: {e}")
    
    return redirect(url_for('admin_messenger_settings'))

@app.route('/admin/messenger_settings/toggle_approval/<int:message_id>', methods=['POST'])
@admin_required
def admin_messenger_toggle_approval(message_id):
    message = Message.query.get_or_404(message_id)
    message.is_approved = not message.is_approved
    try:
        db.session.commit()
        flash(f"Message ID {message.id} approval status toggled.", "success")
    except Exception as e:
        db.session.rollback()
        flash(f"Error toggling message approval: {e}", "danger")
        app.logger.error(f"Error toggling message approval: {e}")
    return redirect(url_for('admin_messenger_settings'))

@app.route('/admin/messenger_settings/delete_message/<int:message_id>', methods=['POST'])
@admin_required
def admin_messenger_delete_message(message_id):
    message = Message.query.get_or_404(message_id)
    try:
        db.session.delete(message)
        db.session.commit()
        flash(f"Message ID {message.id} deleted.", "success")
    except Exception as e:
        db.session.rollback()
