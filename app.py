...
class RawPrompt(db.Model):
   ...
   raw_text = db.Column(db.Text, nullable=False) # Changed from tk.Text to db.Text
...
async def reverse_prompt_endpoint():
    ...
    try:
        inferred_prompt = await asyncio.to_thread(generate_reverse_prompt_async, input_text, language_code)
